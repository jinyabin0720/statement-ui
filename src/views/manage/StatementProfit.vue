<template>
  <el-card style=" border-radius: 0px">
      ...
      <div style="margin-top: 15px">
        <el-col :span="24">
          <div style="float: left">
            <el-form :inline="true" :model="statementProfitForm" class="demo-form-inline">
              <el-form-item>
                &nbsp;&nbsp;&nbsp;&nbsp;期间: <el-input v-model="statementProfitForm.periodCode" placeholder="期间" style="width: 350px" clearable></el-input>
              </el-form-item>
              &nbsp;&nbsp;&nbsp;&nbsp;
              <el-form-item>
                组织名称: <el-input v-model="statementProfitForm.ouName" placeholder="请输入组织名称" style="width: 350px" clearable></el-input>
              </el-form-item>
              <el-form-item>
    ...
    <div style="border:1px solid #d1d1d1; height: 900px">
      <div slot="header" class="clearfix" >
        &nbsp;&nbsp;&nbsp;&nbsp;<i class="el-icon-menu" style="color: #409eff">利润完成情况</i>
        ...
        <el-button type="text"   @click="modifyProfitId(),operationType='update'" style="float: right;">编辑<i class="el-icon-edit-outline" style="color: #409eff">&nbsp;&nbsp;&nbsp;</i></el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        file: {},
        importUrl:'/kpi/statementProfit/importEmp',
    ...
    methods: {
      ...
      search() {
        //alert(this.statementProfitForm.empOrOrg)
        this.axios.post('/kpi/statementProfit/list/', {  //查询
          pageIndex: this.template.pageIndex,
          pageSize: this.template.pageSize,
          periodCode: this.statementProfitForm.periodCode === "" ? null : this.statementProfitForm.periodCode,
          ouName: this.statementProfitForm.ouName === "" ? null : this.statementProfitForm.ouName,
        }).then(data => {
      ...
      add(formName) {  //增加  点击确定触发的方法
        ...
        if (this.operationType == 'insert') {
          this.axios.post('/kpi/statementProfit/save', this.addForm).then(data => {
            if (data && data.status == 200) {
              this.dialogFormVisibless = false;
          ...
          this.axios.put('/kpi/statementProfit/modify', this.addForm).then(data => {
            if (data && data.status == 200) {
              this.dialogFormVisibless = false;
      ...
      modifyProfitId() {
        // let id = this.addForm.analyzeProfitsId;
        // if (id==null&&id==""){
        ...
        this.axios.post('/kpi/statementProfit/findId',{analyzeProfitsId:this.addForm.analyzeProfitsId}  ).then(data => {
          if (data && data.status == 200) {
      ...
      deleteClick() {
        this.axios.post('/kpi/statementProfit/delete/' ,{analyzeProfitsId:this.addForm.analyzeProfitsId}).then(data => {
          if (data && data.status == 200) {
      ...
      selectionChange(row) {   //多选框选中id事件
        this.addForm.analyzeProfitsId = row[0].analyzeProfitsId;
      },
      currentChange(val) {
    ...
    computed: {
      watchSearchName() {
        return this.statementProfitForm.name;
      },
      watchSearchEmp() {
        return this.statementProfitForm.empOrOrg;
      },
      watchSearchDj() {
        return this.statementProfitForm.isDj;
      }
    },
    watch: {}
  }
